package com.stepik.functionalProgramming.Lesson7.Tasks5FunctionalException.part3;
...
class Success<T> implements Try<T> {
    ...
    public <U> Try<U> map(ThrowableFunction<? super T, ? extends U> function) {
        // write your code here
        try {
            return new Success<U>(function.apply(value));
        } catch (Throwable e) {
    ...
    public <U> Try<U> flatMap(ThrowableFunction<? super T, Try<U>> function) {
        // write your code here
        try {
            Try<U> apply = function.apply(value);
            return new Success<U>(apply.get());
        } catch (Throwable e) {
            return new Failure<U>(e);
        }
    }
...
class Failure<T> implements Try<T> {
    ...
    public Try<T> recoverWith(ThrowableFunction<? super Throwable, Try<T>> function) {
        // write your code here
        try {
            Try<T> apply = function.apply(e);
            return new Success<>(apply.get());
        } catch (Throwable ex) {
            return new Failure<>(ex);
        }
    }
