...
/*
The findFirstOrder method takes a list of orders and must return the first / earliest order in the list
according to the value of the deliveryDate field. If the list is empty, the method must return an order
created by its default constructor ("the empty order").
...
class ProcessDeliveryOrders {
    ...
    public static DeliveryOrder findFirstOrder(List<DeliveryOrder> orders) {
        ...
        return orders.stream().min(DeliveryOrder.getComparatorByDeliveryDate()).orElse(new DeliveryOrder());
    }
